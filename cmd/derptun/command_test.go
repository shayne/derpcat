...
func TestRunServePassesTokenAndTCP(t *testing.T) {
	...
	derptunServe = func(ctx context.Context, cfg session.DerptunServeConfig) error {
		if cfg.Token != "dt1_test" {
			t.Fatalf("Token = %q, want dt1_test", cfg.Token)
		}
	...
	code := run([]string{"serve", "--token", "dt1_test", "--tcp", "127.0.0.1:22"}, strings.NewReader(""), &bytes.Buffer{}, &bytes.Buffer{})
	if code != 0 {
...
func TestRunOpenPrintsBindAddress(t *testing.T) {
	...
	code := run([]string{"open", "--token", "dt1_test", "--listen", "127.0.0.1:2222"}, strings.NewReader(""), &bytes.Buffer{}, &stderr)
	if code != 0 {
...
func TestRunConnectRequiresStdio(t *testing.T) {
	var stderr bytes.Buffer
	code := run([]string{"connect", "--token", "dt1_test"}, strings.NewReader(""), &bytes.Buffer{}, &stderr)
	if code != 2 {
