...
func TestEncodeWireFormatContract(t *testing.T) {
	...
	if got, want := len(raw), fixedPayloadSize+len(tok.ShareTargetAddr)+len(tok.DefaultBindHost)+4; got != want {
	...
	if decoded.ShareTargetAddr != tok.ShareTargetAddr || decoded.DefaultBindHost != tok.DefaultBindHost || decoded.DefaultBindPort != tok.DefaultBindPort {
		t.Fatalf("decoded share metadata = %+v, want %+v", decoded, tok)
	}
}
